...
process {
    foreach ($instance in $sqlinstance) {
        ...
        if ($baseaddress -ne $env:COMPUTERNAME) {
            ...
            winrm.cmd id -r:$baseaddress 2>$null | Out-Null
...
try {
    ...
    if ($null -eq $hostname) {
        $hostname = (nbtstat.exe -A $ipaddr | Where-Object {
                $_ -match '\<00\>  UNIQUE'
            } | ForEach-Object {
                $_.SubString(4, 14)
            }).Trim()
...
if ($pscmdlet.ShouldProcess($baseaddress, "Starting $instance in single-user mode")) {
    ...
    try {
        if ($instance.IsLocalHost) {
            $netstart = net.exe start ""$displayname"" /mReset-DbaAdmin 2>&1
            ...
            $netstart = Invoke-Command -ErrorAction Stop -Session $session -ArgumentList $displayname -ScriptBlock {
                net.exe start ""$args"" /mReset-DbaAdmin
            } 2>&1
